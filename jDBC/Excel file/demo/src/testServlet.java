...
//          statement.setFetchSize(15);//Now it's enabled when the query references only a single table in the database.
            ...
            if(lastSQL.equals("RELEASE ALL Excel DATABASE")){
                com.hxtt.sql.excel.ExcelDriver.releaseAll();
                sendLogonHTML();
                return;
            }
    ...
    private void sendLogonHTML()
    {
        ...
        words.addElement("<blockquote><div align=center><center><p>URL: <input type=text name=url size=30 tabindex=4>(e.g., jdbc:Excel:/datafiles)&nbsp;");
        words.addElement("</center></div></blockquote>");
